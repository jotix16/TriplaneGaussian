...
class Transformer1D(BaseModule, MemoryEfficientAttentionMixin):
    ...
    def forward(
        ...
        Args:
            hidden_states -- (B, C, seq_len) -- (`torch.LongTensor` of shape `(batch size, num latent pixels)` if discrete, `torch.FloatTensor` of shape `(batch size, channel, height, width)` if continuous):
                Input `hidden_states`.
            encoder_hidden_states -- (B, seq_len, C) --( `torch.FloatTensor` of shape `(batch size, sequence len, embed dims)`, *optional*):
